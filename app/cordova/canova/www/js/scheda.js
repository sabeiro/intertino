...
var scheda = {
	...
	for (var i=0; i<app.storage.length; i++) {
		...
		li.on("tap",function (){
		    ...
		    var sManu = '';
		    if(scheda.sData.manutenzione == '1'){
			sManu = 'in manutenzione';
		    }
		    var listIt = '<li class="coupon-element" data-theme="c" data-transition="slide">' + scheda.sData.id + ") " + scheda.sData.addetto + " pulì il " + scheda.sData.pullman + " con " + scheda.sData.pulizia + " il " + scheda.sData.date + " " + sManu + '<\/li>';
		    $('#listIt').html(listIt);
		});
	...
	if(isData){
	    $("#btnAggiornatore").html("Manda dati online");
	    $('#btnAggiornatore').css("display","block");
	}
	else{
	    elencoSchede.html("Nessun dato");
	}
    },
    listoreOff: function(){
	for (var i=0; i<app.storage.length; i++) {
	...
	var queryStr = "INSERT INTO wa_pulito (addetto,pullman,pulizia,data,manutenzione) VALUES ('" + sData.addetto + "','" + sData.pullman + "','" + sData.pulizia + "','" + sData.date + "','" + sData.manutenzione + "');";
	//alert(queryStr);
	var request = $.ajax({
	...
	var key = "record_" + scheda.sData.id;
	...
	var queryStr = "INSERT INTO wa_pulito (addetto,pullman,pulizia,data,manutenzione) VALUES ('" + addetto + "','" + pullman + "','" + pulizia + "','" + date + "','" + manutenzione + "');";
	...
	if(pulizia != 'undefined'){
		...
		$.each(data, function(i,el) {
		    var sManu = '';
		    if(el.manutenzione == '1'){
			sManu = 'in manutenzione';
		    }
		    var li = $('<li class="coupon-element" data-theme="c" data-transition="slide">' + el.id + ") " + el.addetto + " pulì il " + el.pullman + " con " + el.pulizia + " il " + el.data + " " + sManu + '<\/li>');
		    li.on("taphold",function (){
			var key = el.id;
			var r = confirm("Confermi eliminazione scheda?");
			if(r==true){
