...
locL.to_csv(baseDir + "raw/location_list.csv",index=False)
...
for i in range(locL.shape[0]):
    ...
    if resq.status_code == 200:
        ...
        dirL2.to_csv(baseDir + "raw/dir_loc"+str(locL['locationId'].iloc[i])+".csv")
    ...
    if resq.status_code == 200:
        ...
        # with gzip.open(baseDir + "out/num_loc"+str(locL['locationId'].iloc[i])+".json.tar.gz",'w') as outfile:
        #     json.dump(numL,outfile,indent=2)

dirT.to_csv(baseDir + "out/dir_loc.csv",index=False)
