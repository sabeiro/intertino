...
token <- POST("http://api.dashboard.ad.dotandad.com:9190/api/v1/token",body=toJSON(cred))
...
fs <- read.csv("log/bkOrderJul.csv",stringsAsFactor=F)
fs <- rbind(fs,read.csv("log/bkOrderAug.csv",stringsAsFactor=F))
fs <- rbind(fs,read.csv("log/bkOrderSept.csv",stringsAsFactor=F))
fs <- rbind(fs,read.csv("log/bkOrderOct.csv",stringsAsFactor=F))
fs <- rbind(fs,read.csv("log/bkOrderNov.csv",stringsAsFactor=F))
fs <- rbind(fs,read.csv("log/bkOrderDec.csv",stringsAsFactor=F))
...
write.csv(fs,"log/bkOrder2016.csv")
